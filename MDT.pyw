...
from PyQt6.QtCore import Qt, QTimer, QPropertyAnimation, QEasingCurve, QRect, QSize, QEventLoop
...
class MDT(QWidget):
    ...
    def init_settings_page(self):
        ...
        card = QWidget(p)
        card.setGeometry(20, 64, 500, 240)
        ...
        make_section_label("ТЕМА ОФОРМЛЕНИЯ", card).setGeometry(20, 116, 300, 18)
        make_field_label("Цвет интерфейса", card).setGeometry(20, 142, 300, 18)

        self.theme_box = styled_combo(card)
        self.theme_box.setGeometry(20, 164, 200, 34)
        ...
        self.save_settings_btn.setGeometry(20, 192, 150, 36)
        self.save_settings_btn.clicked.connect(self.save_settings)

        change_btn = styled_button(p, "🔄  Сменить офицера", False)
        change_btn.setGeometry(20, 320, 200, 36)
        change_btn.clicked.connect(self._change_officer)

        ver_lbl = QLabel(f"MDT версия {CURRENT_VERSION}", p)
        ver_lbl.setGeometry(20, 372, 300, 24)
...
<span style='color: #f59e0b; font-size: 12px;'>⚖ {pdata.get('punishment','')}</span>
    ...
    def _change_officer(self):
        ...
        # Предзаполняем текущими данными
        ...
        # Блокирующий мини-цикл — ждём закрытия диалога без QTimer
        inner_loop = QEventLoop()

        def on_close(event):
            QWidget.closeEvent(login, event)
            inner_loop.quit()

        login.closeEvent = on_close
        inner_loop.exec()
